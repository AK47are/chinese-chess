...
void Widget::initChessBoard() {
    ...
    QPushButton* robot = msg_box.addButton("人机对战", QMessageBox::ActionRole);
    ...
    if (msg_box.clickedButton() == offline) {
        ...
        if (msg_box.clickedButton() == create) {
            ...
            new NetChessBoard(ui->chessboard, ui->chess_text, ui->reset, ui->back,
            ...
            if (is_ip && !ip.isNull() && is_port && is_convert && port <= 65535) {
                new NetChessBoard(ui->chessboard, ui->chess_text, ui->reset,
    ...
    } else if (msg_box.clickedButton() == robot) {
        new RobotChessBoard(ui->chessboard, ui->chess_text, ui->reset, ui->back);
    } else {
        exit(0);
    }
}
